...
function install(name, target, options, seen) {
  ...
  .then(function(located) {
    ...
    // check if the package version is linked
    // (install relinking)
    if (target.version && !options.peerParent)
      return asp(fs.lstat)(target.getPath())
      .then(function(stats) {
  ...
  .then(function(exactLink) {
    // if linked, retain the current resolution
    if (exactLink) {
      resolution = target;
      linked = true;
      return;
