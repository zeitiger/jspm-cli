...
exports.install = function(targets, options) {
  ...
  .then(function() {
    ...
    return Promise.all(Object.keys(targets).map(function(name) {
      ...
      // set config.dev per package
      if (bulk)
        opts.dev = !config.pjson.dependencies[name];

      // installing a dev dependency that is already a normal dependency stays a normal dependency
      else if (opts.dev && config.pjson.dependencies[name])
        opts.dev = false;
...
/*
 * install('jquery', 'jquery', { latest, lock, parent, inject, unlink, override, peerParent } [, seen])
 *
 * Install modes:
...
function install(name, target, options, seen) {
  ...
  .then(function(located) {
    ...
    if (!options.peerParent || options.lock)
      return located;
    ...
    if (!existing || existing.name == target.name && semver.match(target.version, existing.version))
      return located;

    // conflict resolution!
    ui.log('info', '\nPeer dependency conflict for %' + name + '%:\n' +
        '  Currently installed as `' + existing.exactName + '`.\n' +
        '  Package `' + options.peerParent + '` requires `' + located.exactName + '`.');
  ...
  .then(function(located) {
    ...
    // get the natural installed match
    var installedMatch = getInstalledMatch(target, options.parent, name);

    if (!installedMatch)
      return;

    // check if it is linked (if so, lock)
    return asp(fs.lstat)(installedMatch.getPath())
    .then(function(stats) {
    ...
    .then(function(isLinked) {
      if (!isLinked)
        return options.lock && installedMatch;

      linked = true;
      return installedMatch;
    });
  })
  .then(function(_resolution) {
    if (_resolution) {
  ...
  function storeResolution() {
    ...
    if (!options.parent) {
      ...
      // peer deps only stored if already there by name
      if (!options.peerParent || config.pjson.dependencies[name]) {
        if (options.dev)
    ...
    dependencyDownloads = (dependencyDownloads || Promise.resolve()).then(function() {
      return Promise.all(Object.keys(depMap).map(function(dep) {
        ...
        return install(dep, depMap[dep], {
          ...
          lock: options.lock,
          parent: !isPeer && resolution.exactName,
          inject: options.inject,
          quick: options.quick,
          peerParent: isPeer && resolution.exactName
        }, seen);
      }));
    });
  }
...
function logInstall(name, target, resolution, linked, options) {
  ...
  if (options.parent)
    ...
    ui.log('ok', verb + '%' + name + '% ' + (options.peerParent ? 'peer ' : '') + 'to `' + target.exactName + '` (' + actual + ')');
}
